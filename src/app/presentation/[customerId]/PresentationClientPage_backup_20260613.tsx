...
export default function PresentationClientPage({ initialData, customerId }: PresentationClientPageProps) {
  ...
  // --- 仕様切替パネル タブステート ---
  const [activeTab, setActiveTab] = useState<"spec" | "assets" | "vr">("spec");
  ...
  return (
    <div className="presen-container">
      ...
      <div className="a3-board-wrapper">
        ...
        {/* ==================== 0面目: 表紙（カバーページ） ==================== */}
        <article className="paper-sheet cover-sheet">
          <div className="cover-content">
            ...
            <div className="cover-footer">
              <div className="cover-date">{new Date().toLocaleDateString('ja-JP', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
            ...
            <div className="perspective-showcase-split">
              <div className="perspective-showcase aspect-16-9" style={{ position: "relative", width: "100%", aspectRatio: "16 / 9", overflow: "hidden", borderRadius: "8px" }}>
                <span className={`perspective-badge ${showVr ? "vr-badge" : ""}`}>
                  {showVr ? "🕶️ 360°VRビュー（ジャイロ連動）" : "✨ ご提案内観イメージ (選択中)"}
                </span>
            ...
            <div style={{ marginTop: "0.6rem", borderTop: "1.5px solid #e2e8f0", paddingTop: "0.6rem" }}>
              <div style={{ display: "grid", gridTemplateColumns: "7fr 3fr", gap: "1rem" }}>
                ...
                <div>
                  ...
                  <div style={{ border: "1px solid #cbd5e1", borderRadius: "6px", background: "#f8fafc", padding: "0.5rem", height: "300px", display: "flex", flexDirection: "column", gap: "0.4rem", boxSizing: "border-box" }}>
      ...
      <div className={`floating-control-panel ${showControls ? "show" : ""}`}>
        ...
        <div className="panel-tabs">
          ...
          <button 
            type="button" 
            className={`panel-tab-btn ${activeTab === "vr" ? "active" : ""}`}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.8rem" }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", padding: "0.8rem", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.08)" }}>
              ...
              {isFetchingGithub ? (
                ...
                <div style={{ marginTop: "0.5rem", borderTop: "1px dashed rgba(255,255,255,0.1)", paddingTop: "0.5rem" }}>
                  <span style={{ fontSize: "0.7rem", fontWeight: "bold", color: "#cbd5e1" }}>✨ 検出された画像 (役割をアサイン)：</span>
                  <div style={{ display: "flex", flexDirection: "column", gap: "3px", marginTop: "0.25rem", maxHeight: "150px", overflowY: "auto", background: "rgba(0,0,0,0.15)", padding: "0.25rem", borderRadius: "4px" }}>
                    {githubImages.map((img, idx) => (
                      <div key={idx} style={{ display: "flex", gap: "0.4rem", background: "rgba(255,255,255,0.02)", padding: "0.25rem", borderRadius: "4px", alignItems: "center", border: "1px solid rgba(255,255,255,0.05)" }}>
                        <img src={img.downloadUrl} alt={img.name} style={{ width: "30px", height: "30px", objectFit: "cover", borderRadius: "3px", background: "#f1f5f9" }} />
                        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
                          <span style={{ fontSize: "0.6rem", color: "#f8fafc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={img.name}>{img.name}</span>
                          <select
                            value={getAssignedRole(img.downloadUrl)}
                            onChange={(e) => handleAssignImage(e.target.value, img.downloadUrl)}
                            style={{ background: "#070a13", color: "#e2b83b", border: "1px solid rgba(226, 184, 59, 0.4)", borderRadius: "3px", fontSize: "0.6rem", padding: "1px", outline: "none", cursor: "pointer", fontWeight: "bold", marginTop: "2px" }}
                          >
                            <option value="">🚫 なし</option>
                            <option value="exterior">🏡 外観パース画像</option>
                            <option value="elevation">📐 立面図画像</option>
                            <option value="beforeFloorplan">🏚️ Before平面図</option>
                            <option value="afterFloorplan">🛋️ After平面図</option>
                            <option value="interiorModern">✨ 内観1(モダン)</option>
                            <option value="interiorNatural">✨ 内観2(ナチュラル)</option>
                            <option value="vrImageA">🕶️ VR画像A (1枚目)</option>
                            <option value="vrImageB">🕶️ VR画像B (2枚目)</option>
                          </select>
                        </div>
                      </div>
                    ))}
                ...
                <div style={{ fontSize: "0.65rem", color: "#94a3b8", fontStyle: "italic", marginTop: "0.5rem" }}>指定フォルダ内に画像ファイルが見つかりません。</div>
              )}
            </div>

            {/* パース・図面画像パス手動設定 */}
            <div className="control-group">
              <span className="control-label">🎨 パース・図面パス手動入力</span>
        ...
        {activeTab === "vr" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <div className="control-group">
              <span className="control-label">🕶️ 360°VRパノラマ画像設定</span>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                  <span style={{ fontSize: "0.65rem", color: "#cbd5e1", fontWeight: "bold" }}>VR用GitHubフォルダ名</span>
                  <div style={{ display: "flex", gap: "0.3rem" }}>
                    <input 
                      ...
                      style={{ flex: 1, padding: "0.35rem 0.5rem", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.15)", background: "#0f172a", color: "#ffffff", fontSize: "0.75rem" }}
                    />
                    <button
                      type="button"
                      onClick={() => fetchVrFolderImages(vrFolder || customerId)}
                      style={{ background: "#e2b83b", color: "#070a13", border: "none", padding: "0.35rem 0.6rem", borderRadius: "6px", fontWeight: "bold", cursor: "pointer", fontSize: "0.7rem" }}
                    >
                      スキャン
                    </button>
                  </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                  <span style={{ fontSize: "0.65rem", color: "#cbd5e1", fontWeight: "bold" }}>パノラマ画像A (1枚目)</span>
                  {isFetchingVrImages ? (
                    <span style={{ fontSize: "0.7rem", color: "#94a3b8" }}>スキャン中...</span>
                  ) : vrFolderImages.length > 0 ? (
                    <select
                      value={vrImageA}
                      onChange={(e) => setVrImageA(e.target.value)}
                      style={{ width: "100%", padding: "0.35rem 0.5rem", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.15)", background: "#0f172a", color: "#ffffff", fontSize: "0.75rem" }}
                    >
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "2px" }}>
                  <span style={{ fontSize: "0.65rem", color: "#cbd5e1", fontWeight: "bold" }}>パノラマ画像B (2枚目)</span>
                  {isFetchingVrImages ? (
                    <span style={{ fontSize: "0.7rem", color: "#94a3b8" }}>スキャン中...</span>
                  ) : vrFolderImages.length > 0 ? (
                    <select
                ...
                {vrImagesError && <div style={{ fontSize: "0.65rem", color: "#ef4444" }}>⚠️ VR画像のロードに失敗しました。</div>}
              </div>
            </div>
