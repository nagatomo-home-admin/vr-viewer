...
export default function PresentationPortalClient({ initialList }: PortalClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetFormFields();
      alert("保存が完了しました！");
      router.push(`/presentation/${trimmedId}`);
    } catch (e) {
      console.error(e);
        ...
        {showForm && (
          <form className="form-section" onSubmit={handleSubmit}>
            ...
            <div className="form-grid">
              ...
              {/* パース画像・図面設定 */}
              <div className="form-group-full" style={{ borderTop: "1px dashed rgba(255,255,255,0.1)", paddingTop: "1rem" }}>
                <h3>🖼️ パース画像・図面設定 (GitHub連携 & 自動割り当て対応)</h3>
              </div>

              {/* GitHub 連携設定 */}
              <div className="form-group-full" style={{ background: "rgba(255,255,255,0.03)", padding: "1.25rem", borderRadius: "10px", border: "1px solid rgba(255,255,255,0.08)", marginBottom: "1rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
                  <h4 style={{ color: "#e2b83b", fontSize: "0.9rem", fontWeight: "700", margin: 0 }}>🤖 GitHub画像フォルダ自動アサイン連携</h4>
                  <button 
                    type="button" 
                  ...
                  >
                    {showGithubSettings ? "⚙️ リポジトリ連携設定を隠す ▴" : "⚙️ リポジトリ連携設定を開く ▾"}
                  </button>
                </div>

                {/* GitHub共通リポジトリ設定 (アコーディオン) */}
                {showGithubSettings && (
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "0.75rem", marginBottom: "1rem", padding: "1rem", background: "rgba(0,0,0,0.2)", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.05)", animation: "slide-down 0.2s ease-out" }}>
                    <div className="form-group">
                      <label style={{ fontSize: "0.75rem" }}>リポジトリ所有者 (Owner)</label>
                      <input 
                        type="text" 
                        className="form-input" 
                        style={{ padding: "0.45rem 0.65rem", fontSize: "0.8rem" }}
                        placeholder="例: shogo-nagatomo"
                        value={githubOwner}
                    ...
                    <div className="form-group">
                      <label style={{ fontSize: "0.75rem" }}>リポジトリ名 (Repo)</label>
                      <input 
                        type="text" 
                        className="form-input" 
                        style={{ padding: "0.45rem 0.65rem", fontSize: "0.8rem" }}
                        placeholder="例: renovation-images"
                        value={githubRepo}
                    ...
                    <div className="form-group">
                      <label style={{ fontSize: "0.75rem" }}>親フォルダパス (任意)</label>
                      <input 
                        type="text" 
                        className="form-input" 
                        style={{ padding: "0.45rem 0.65rem", fontSize: "0.8rem" }}
                        placeholder="例: assets/ (空でも可)"
                        value={githubParentPath}
                        onChange={(e) => setGithubParentPath(e.target.value)}
                        disabled={isSubmitting}
                      />
                    </div>
                    <div className="form-group" style={{ justifyContent: "flex-end" }}>
                      <button
                        ...
                        disabled={isFetchingFolders || isSubmitting}
                        style={{
                ...
                <div style={{ marginTop: "0.75rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  <span style={{ fontSize: "0.75rem", fontWeight: "bold", color: "#cbd5e1" }}>📂 お施主様フォルダを選択（クリックで自動ロード）：</span>
                  {isFetchingFolders ? (
                    <div style={{ fontSize: "0.8rem", color: "#e2b83b", padding: "0.5rem 0" }}>🔄 GitHubからお施主様フォルダ一覧を読み込んでいます...</div>
                  ) : githubFolders.length > 0 ? (
                    <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", maxHeight: "120px", overflowY: "auto", paddingBottom: "0.25rem" }}>
                      {githubFolders.map((folder) => {
                        const isActive = githubFolder === folder.name;
                        return (
                          <button
                            key={folder.name}
                            type="button"
                            onClick={() => handleSelectFolderTab(folder.name)}
                            style={{
                              background: isActive ? "linear-gradient(135deg, #e2b83b 0%, #c69b6b 100%)" : "rgba(255,255,255,0.06)",
                              color: isActive ? "#070a13" : "#cbd5e1",
                              border: isActive ? "1px solid #e2b83b" : "1px solid rgba(255,255,255,0.1)",
                    ...
                    <div style={{ fontSize: "0.8rem", color: "#94a3b8", padding: "0.5rem 0", fontStyle: "italic" }}>
                      リポジトリ内にフォルダが見つかりません。連携設定の「所有者名」「リポジトリ名」が正しいかご確認ください。
                    </div>
                  )}
                </div>

                {githubError && <div style={{ color: "#fca5a5", fontSize: "0.8rem", fontWeight: "bold", marginTop: "0.5rem" }}>⚠️ {githubError}</div>}

                {/* 自動取得した画像の一覧 ＆ 役割アサイン */}
                {isFetchingGithub ? (
                  <div style={{ marginTop: "1rem", borderTop: "1px dashed rgba(255,255,255,0.1)", paddingTop: "0.75rem", fontSize: "0.8rem", color: "#e2b83b" }}>
                    🔄 選択したフォルダから画像を読み込んでいます...
                  </div>
                ) : githubImages.length > 0 && (
                  <div style={{ marginTop: "1rem", borderTop: "1px dashed rgba(255,255,255,0.1)", paddingTop: "0.75rem" }}>
                    <span style={{ fontSize: "0.85rem", fontWeight: "bold", color: "#cbd5e1" }}>✨ 検出された画像（役割を割り当ててください）：</span>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "0.5rem", marginTop: "0.5rem", maxHeight: "200px", overflowY: "auto", paddingRight: "0.5rem" }}>
                      {githubImages.map((img, idx) => (
                        <div key={idx} style={{ display: "flex", gap: "0.5rem", background: "rgba(0,0,0,0.2)", padding: "0.4rem", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.05)", alignItems: "center" }}>
                          <img 
                            src={img.downloadUrl} 
                            alt={img.name} 
                            style={{ width: "45px", height: "45px", objectFit: "cover", borderRadius: "4px", background: "#f1f5f9" }} 
                          />
                          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.15rem", minWidth: 0 }}>
                            <span style={{ fontSize: "0.7rem", color: "#f8fafc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={img.name}>{img.name}</span>
                            <select
                              value={getAssignedRole(img.downloadUrl)}
                              onChange={(e) => handleAssignImage(e.target.value, img.downloadUrl)}
                              style={{
                                background: "#070a13",
                                color: "#e2b83b",
                            ...
                            >
                              ...
                              <option value="exterior">🏡 外観パース画像</option>
                              <option value="elevation">📐 立面図画像</option>
                              <option value="beforeFloorplan">🏚️ Before平面図画像</option>
                              <option value="afterFloorplan">🛋️ After平面図画像</option>
                              <option value="interiorModern">✨ 内観1(モダン)</option>
                              <option value="interiorNatural">✨ 内観2(ナチュラル)</option>
                              <option value="vrImageA">🕶️ VRパノラマ画像A (1枚目)</option>
                              <option value="vrImageB">🕶️ VRパノラマ画像B (2枚目)</option>
                            </select>
                          </div>
                        </div>
                      ))}
              ...
              {/* 個別の画像パス入力項目 (スキャン結果がここに自動連動します) */}
              <div className="form-group">
                <label>外観パース画像パス (標準: /luxury_exterior.png)</label>
                <input 
                  type="text" 
                  className="form-input" 
                  value={exteriorImg}
                  onChange={(e) => setExteriorImg(e.target.value)}
                  disabled={isSubmitting}
                />
              </div>
              <div className="form-group">
                <label>立面図画像パス (空欄の場合は標準CADイメージを表示)</label>
                <input 
                  type="text" 
                  className="form-input" 
                  placeholder="例: luxury_elevation.png"
                  value={elevationImg}
                  onChange={(e) => setElevationImg(e.target.value)}
                  disabled={isSubmitting}
                />
              </div>
              <div className="form-group">
                <label>Before 平面図画像パス (空欄の場合は標準の旧間取りを表示)</label>
                <input 
                  type="text" 
                  className="form-input" 
                  placeholder="例: before_floorplan.png"
                  value={beforeFloorplanImg}
                  onChange={(e) => setBeforeFloorplanImg(e.target.value)}
                  disabled={isSubmitting}
                />
              </div>
              <div className="form-group">
                <label>After 平面図画像パス (空欄の場合は標準の提案間取りを表示)</label>
                <input 
                  type="text" 
                  className="form-input" 
                  placeholder="例: after_floorplan.png"
                  value={afterFloorplanImg}
                  onChange={(e) => setAfterFloorplanImg(e.target.value)}
                  disabled={isSubmitting}
                />
              </div>
              <div className="form-group">
                <label>内観パース画像1: モダン (標準: /interior_modern.png)</label>
                <input 
                  type="text" 
                  className="form-input" 
                  value={interiorModernImg}
                  onChange={(e) => setInteriorModernImg(e.target.value)}
                  disabled={isSubmitting}
                />
              </div>
              <div className="form-group">
                <label>内観パース画像2: ナチュラル (標準: /interior_natural.png)</label>
                <input 
                  type="text" 
                  className="form-input" 
                  value={interiorNaturalImg}
                  onChange={(e) => setInteriorNaturalImg(e.target.value)}
                  disabled={isSubmitting}
                />
              </div>
              ...
              {/* パノラマVR設定 */}
              <div className="form-group-full" style={{ borderTop: "1px dashed rgba(255,255,255,0.1)", paddingTop: "1rem" }}>
                <h3>🕶️ パノラマVR設定 (Nagatomo VR Viewer 連携)</h3>
              </div>
              <div className="form-group-full">
                <label>パノラマVR用GitHubフォルダ名（空欄の場合は顧客IDを使用。例: r_006_tateno）</label>
                <input 
                  type="text" 
                  className="form-input" 
              ...
              <div className="form-group">
                <label>パノラマ画像A (1枚目/Before 等。例: sakaguchi_1_ldk_1.jpeg)</label>
                <input 
                  type="text" 
                  className="form-input" 
              ...
              <div className="form-group">
                <label>パノラマ画像B (2枚目/After 等。例: sakaguchi_2_ldk_1.jpeg)</label>
                <input 
                  type="text" 
                  className="form-input" 
          ...
          <div className="client-grid">
            {list.filter(client => 
              ...
              list.filter(client => 
                ...
                <div 
                  onClick={() => router.push(`/presentation/${client.id}`)}
                  ...
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", width: "100%" }}>
                      ...
                      <div style={{ display: "flex", gap: "0.35rem", zIndex: 10 }}>
                        ...
                        <button 
                          title="複製" 
                          onClick={(e) => handleDuplicateClick(e, client)} 
                          className="card-action-btn"
                        ...
                        <button 
                          title="削除" 
                          onClick={(e) => handleDeleteClick(e, client.id, client.clientName)} 
                          className="card-action-btn"
